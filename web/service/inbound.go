...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	if inbound.Enable {
		...
		if err1 != nil {
			...
			needRestart = true
		} else if err1 = s.xrayApi.Init(p.GetAPIPort()); err1 != nil {
			...
			if err1 == nil {
				...
				logger.Warning("Unable to add inbound by api:", err1)
				needRestart = true
			}
			s.xrayApi.Close()
		}
...
func (s *InboundService) AddInboundClient(data *model.Inbound) (bool, error) {
	...
	needRestart := false
	apiReady := s.xrayApi.Init(p.GetAPIPort()) == nil
	if !apiReady {
		needRestart = true
	}
	for _, client := range clients {
