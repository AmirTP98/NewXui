...
// SyncConfig updates the stored process config from the database so that
// RestartXray(false) does not see a stale diff after successful hot-reloads.
func (s *XrayService) SyncConfig() {
	if p == nil {
		return
	}
	xrayConfig, err := s.GetXrayConfig()
	if err != nil {
		logger.Debug("SyncConfig: failed to load xray config:", err)
		return
	}
	p.SetConfig(xrayConfig)
}
